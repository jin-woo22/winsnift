...
class AuthController extends Controller
{
        ...
        if (Auth::attempt($credentials + ['is_activated' => true])) {
            ...
            if($request->query('platform')){
                return match (auth()->user()->role->name) {
                    'admin' => to_route('admin.locations.create', ['platform' => 'mobile']),
                    'user' => to_route('user.plantations.index')
                }; 
            }
            ...
            return redirect('/login')->with('error', 'Oops! The Username / Password do not match');
        }
    }
    ...
    public function logout(Request $request)
    {
        ...
        if($request->query('platform')){
            return to_route('auth.login_mobile');
        }
